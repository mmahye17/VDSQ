...
def generate_anti(N, d, precision=2):
    ...
    for i in range(N):
        ...
        plane_distance = np.random.normal(loc=0.5, scale=0.04)
        # 第一个维度在 [0, 1) 内均匀分布
        ...
        while alpha:
            ...
            if len(alpha) >= 2:
                ...
                for i in range(1, len(alpha)):
                    ...
                    # 将 sum_val 转换为字符串，保留10位小数
                    sum_val_str = f"{sum_val:.10f}"
                    # 使用 keys[key_idx] 加密 sum_val_str
                    enc_sum_val = encrypt(sum_val_str, keys[key_idx])
                    sum_idx = cantor_pairing(first_idx, alpha[i][1])
                    hash_tables[dim][(first_idx, alpha[i][1])] = (enc_sum_val, key_idx)
            if len(alpha) >= 3:
                ...
                for i in range(1, len(alpha) - 1):
                    ...
                    hash_tables[dim][(last_idx, alpha[i][1])] = (enc_sum_val, key_idx)
