...
def build_merkle_tree(leaves: List[bytes]) -> bytes:
    """
    构建 Merkle 树，返回根哈希。
    输入：leaves 是 SHA-256 摘要（字节串列表）。
    输出：Merkle 树的根哈希（32 字节）。
    """
    # 确保每个叶子节点是 32 字节的 SHA-256 哈希值
    for leaf in leaves:
        if len(leaf) != 32:
...
def key_generation():
    # 使用 secrets 模块生成安全的随机数作为私钥
    # 确保私钥在 [1, curve_order-1] 范围内
