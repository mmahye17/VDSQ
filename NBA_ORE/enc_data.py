import gc
import pandas as pd  # [修改]：添加 pandas 导入以支持 Excel 文件读取
import numpy as np
...
try:
    ...
    if df.shape[0] < N or df.shape[1] < d:
        raise ValueError(f"Excel 数据不足：需要至少 {N} 行 {d} 列，实际为 {df.shape[0]} 行 {df.shape[1]} 列")
    indep_data = df.iloc[:N, :d].to_numpy()
    if indep_data.shape != (N, d):
        raise ValueError(f"预期数据形状为 ({N}, {d})，实际为 {indep_data.shape}")
    if not np.all((indep_data >= 0) & (indep_data < 10000) & (np.mod(indep_data, 1) == 0)):
        raise ValueError("数据必须是 [0, 9999] 范围内的整数")
...
"""
corr_for_sum= store_plaintext_and_index_for_sum(corr_avl_trees, d)

anti_for_sum = store_plaintext_and_index_for_sum(anti_avl_trees, d)
"""
